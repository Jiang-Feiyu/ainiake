<template>
  <div
    v-if="editorLayout.length && selectCompont && selectCompont.id"
    :key="selectCompont.id"
    class="option-editor"
  >
    <h3>组件设置</h3>
    <div class="options-list">
      <el-row :gutter="20" class="option-row">
        ...
        <el-col :span="18">
          <el-input v-model.number="selectCompont.name" size="mini"> </el-input>
        </el-col>
      </el-row>
      <el-row :gutter="20" class="option-row">
      ...
      <el-row
        v-for="item in componentOptions.filter((v) => !v.childComponent)"
        :key="item.style_id"
        class="option-row"
      >
  ...
  <div v-else class="option-editor">未选中组件</div>
</template>
<script>
